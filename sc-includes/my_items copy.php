...
<?php if(isset($sc_msg)): ?>
	<div class="info_valid"><?= $sc_msg?></div>
<?php endif ?>
...
$user_ads= rawQuerySQL("SELECT * FROM sc_ad WHERE active = 1 AND (trash = 0 OR motivo = $motivo OR motivo = $motivo1 OR (motivo = $motivo2 AND date_trash > $lapso) OR (motivo = $motivo3 AND date_trash > $lapso) ) AND ID_user = '".$_SESSION['data']['ID_user']."'  ORDER BY date_ad DESC");
...
<div class="my_items_search">
	...
	<div class="user_info">
		...
		<?php if($_SESSION['data']['rol'] == UserRole::Publicista || $_SESSION['data']['rol'] == UserRole::Centro): ?>
		  ...
		  <?php else: ?>
			<img src="<?=Images::getImage("super-descuento.png")?>" class="discount_ads" alt="publicista_design">
		<?php endif ?>
	</div>
	<div class="item_info">
...
<li class="item">
	...
	<?php if($ad['ad']['trash'] == 1): ?>
		<div class="cover">
				<?php if($ad['ad']['motivo'] == Motivo::Desactivado): ?>
					<h6>Tu anuncio ha sido desactivado. Entra en contacto con nosotros.</h6>
					<p><?=$ad['ad']['trash_comment']?></p>
				<?php endif ?>
	...
	<table><tr><td>
	<style>.mySlides<?=$ad['ad']['ID_ad']?> {display:none;}</style>
	<div class="img-item-content">
		<div class="w3-content w3-display-container">
			<a href="<?=urlAd($ad['ad']['ID_ad'])?>" target="_blank" title="<?=stripslashes($ad['ad']['title'])?>">
			<? if($image){ 
				foreach ($ad['images'] as $foto) { ?>
					<div class="imageAditem <?=getImgOrientation($foto['name_image'])?> mySlides<?=$ad['ad']['ID_ad']?>">
						<div class="bg-img-item" style="background: url(<?=IMG_ADS;?><?=$foto['name_image']?>);"></div>
			...
			<button type="button" class="w3-button w3-black w3-display-left" onclick="plusDivs<?=$ad['ad']['ID_ad']?>(-1)"><i class="fa fa-chevron-left"></i></button> 
			<button type="button" class="w3-button w3-black w3-display-right" onclick="plusDivs<?=$ad['ad']['ID_ad']?>(1)"><i class="fa fa-chevron-right"></i></button>
			<?}?>
		</div>
	</div>
	<script>
	var slideIndex = 1;
	showDivs<?=$ad['ad']['ID_ad']?>(slideIndex);
	function plusDivs<?=$ad['ad']['ID_ad']?>(n) {
			showDivs<?=$ad['ad']['ID_ad']?>(slideIndex += n);
		}
		function showDivs<?=$ad['ad']['ID_ad']?>(n) {
				var i;  var x = document.getElementsByClassName(<? echo '"mySlides'.$ad['ad']['ID_ad'].'"';?>);
	...
	<span class="text2Ad">
		<?php if(isMobileDevice()): ?>
			...
			<?php else: ?>
				<span class="short">
					<?
						echo mb_strimwidth($ad['ad']['texto'], 0, 340, "...");
						//echo $ad['ad']['texto'];
					?>
			</span>
			<span class="open hidden">
...
<div class="options_my_item <?=$ad['ad']['trash'] == 1 ? 'pb-5 pb-md-0': '' ?>">
	...
	<? }?>
	<li class="item_responsive ">
	<a href="javascript:void(0);" class="delete_ad_link" onclick="setDeleteAd(<?=$ad['ad']['ID_ad']?>)" ><?=$language['my_items.opt_delete']?></a></li>
	<? if(!$review_on){
		...
		<?php if($pending == 0): ?>
			<li class="item_image r-hide">
				<a class="" href="javascript:void(0);" onclick="openPayment(<?=$ad['ad']['ID_ad']?>)" >
					<img src="<?=Images::getImage('boton-servicios.webp')?>?t" alt="servicios" height="50px">
				</a>
			</li>
...
<div class="pay_my_item <?=$ad['ad']['trash'] == 1 ? 'position-absolute': 'pt-2' ?>" id="pay_my_item_<?=$ad['ad']['ID_ad']?>">
			...
			<?php if(!$AD_DISCARD): ?>
				...
				<? if(!$review_on){?>
					<?php if($ad['ad']['ID_order'] == 0){ ?>
						...
						<?php endif ?>
					<?php }else{ ?>
						<li class="item_responsive item-float"><a class="btn-green" href="javascript:void(0)" onclick="orderPayment(<?=$ad['ad']['ID_order']?>)">Activar</a></li>
					<?php } ?>
				...
				<? }?>
				<li class="item_responsive <?=$ad['ad']['ID_order'] != 0 && !$review_on ? 'item-float second' : ''?>"><a href="javascript:void(0);" class="delete_ad_link <?=$ad['ad']['ID_order'] != 0 && !$review_on ? 'btn-pink' : ''?>" onclick="setDeleteAd(<?=$ad['ad']['ID_ad']?>)">Eliminar anuncio</a></li>
				<?php if($pending == 0): ?>
